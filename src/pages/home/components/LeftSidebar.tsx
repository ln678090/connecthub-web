...
const navItemsTop = [
    ...
    { icon: MessageCircle, label: 'Tin nhắn',   path: 'https://chat-real-time-7qoyl3xqt-ln678090s-projects.vercel.app/' },
];
...
export default function LeftSidebar({}: LeftSidebarProps) {
   ...
   const renderNavItem = ({ icon: Icon, label, path }: any) => {
    ...
    const handleClick = () => {
        if (isExternal) {
            window.location.href = path; // 👉 chuyển trang khác origin
        } else {
            navigate(path); // 👉 route nội bộ
        }
    };
