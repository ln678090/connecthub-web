    ...
    return (
        <>
            ...
            <div className="flex min-h-screen items-center justify-center bg-[#f3f4f6] p-4 font-sans text-gray-900">
                {/*
                  Sửa height thành min-h để giãn nở trên mobile nếu content quá dài.
                  Sửa layout cho Responsive.
                */}
                <div className="relative w-full max-w-[1100px] min-h-[600px] md:h-[650px] overflow-hidden rounded-3xl md:rounded-[2.5rem] bg-white shadow-2xl flex flex-col md:block">

                    {/* ======================================= */}
                    {/* PANNEL 1: FORM ĐĂNG KÝ */}
                    {/* ======================================= */}
                    <div
                        className={`absolute top-0 left-0 w-full md:w-1/2 h-full flex flex-col justify-center p-8 md:p-16 transition-all duration-700 ease-in-out bg-white
                        ${isRegister
                            ? 'md:translate-x-full translate-x-0 opacity-100 z-10'
                            : 'md:translate-x-0 -translate-x-full opacity-0 z-0 pointer-events-none'
                        }`}
                    >
                        <div className="flex flex-col justify-center max-w-sm mx-auto w-full">
                            <div className="mb-6 flex justify-center">
                                <img src={logo} alt="ConnectHub Logo" className="h-20 md:h-31 w-auto object-contain mix-blend-multiply" />
                            </div>
                            ...
                            <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="flex flex-col gap-4 md:gap-5">
                                <div>
                                    <input
                                        ...
                                        className="w-full rounded-xl bg-gray-100/80 px-4 md:px-5 py-3 md:py-3.5 text-sm outline-none transition focus:bg-white focus:ring-2 focus:ring-[#2e62a0]"
                                    />
                                    {registerForm.formState.errors.fullName && <p className="mt-1 text-xs text-red-500">{registerForm.formState.errors.fullName.message}</p>}
                                </div>
                                <div>
                                    <input
                                ...
                                {serverError && isRegister && <p className="text-center text-sm font-semibold text-red-500">{serverError}</p>}

                                <button
                                    disabled={registerForm.formState.isSubmitting}
                                    className="mt-2 w-full rounded-xl bg-gradient-to-r from-[#2e62a0] to-[#71bc59] p-3 md:p-3.5 font-bold tracking-wide text-white transition hover:opacity-90 shadow-md hover:shadow-lg"
                                >
                                    {registerForm.formState.isSubmitting ? 'ĐANG XỬ LÝ...' : 'ĐĂNG KÝ'}
                                </button>
                            </form>

                            {/* Nút chuyển đổi giao diện dành riêng cho Mobile */}
                            <div className="mt-6 text-center md:hidden">
                                <p className="text-sm text-gray-600">
                                    Đã có tài khoản?{' '}
                                    <button onClick={toggleMode} type="button" className="font-bold text-[#2e62a0] hover:text-[#71bc59]">Đăng nhập</button>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        className={`absolute top-0 left-0 w-full md:w-1/2 h-full flex flex-col justify-center p-8 md:p-16 transition-all duration-700 ease-in-out bg-white
                        ${isRegister
                            ? 'md:translate-x-[100%] translate-x-full opacity-0 z-0 pointer-events-none'
                        ...
                        <div className="flex flex-col justify-center max-w-sm mx-auto w-full">
                            ...
                            <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="flex flex-col gap-4 md:gap-5">
                                ...
                                <div className="text-right mt-1">
                                    <a href="#" className="text-sm font-semibold text-[#2e62a0] hover:text-[#71bc59] transition-colors">Quên mật khẩu?</a>
                                </div>
                            ...
                            <div className="mt-6 text-center md:hidden">
                                <p className="text-sm text-gray-600">
                                    Người mới à?{' '}
                                    <button onClick={toggleMode} type="button" className="font-bold text-[#71bc59] hover:text-[#2e62a0]">Tạo tài khoản</button>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* ======================================= */}
                    {/* PANNEL 3: LỚP OVERLAY TRƯỢT (CHỈ HIỆN TRÊN DESKTOP) */}
                    {/* ======================================= */}
                    <div
                    ...
                    >
                        ...
                        >
                            {/* Overlay Trái */}
                            <div className={`absolute top-0 left-0 flex h-full w-1/2 flex-col items-center justify-center px-16 text-center transition-transform duration-700 ease-in-out ${
                                isRegister ? 'translate-x-0' : '-translate-x-[20%]'
                            }`}>
                                <h2 className="mb-6 text-4xl font-extrabold text-white drop-shadow-md tracking-tight">Đã có tài khoản?</h2>
                                ...
                                <button onClick={toggleMode} className="rounded-full border-2 border-white/80 bg-white/10 backdrop-blur-sm px-12 py-3.5 font-bold uppercase tracking-wider text-white transition hover:bg-white hover:text-[#2e62a0] shadow-xl">
                                    Đăng nhập
                                </button>
                            </div>

                            {/* Overlay Phải */}
                            <div className={`absolute top-0 right-0 flex h-full w-1/2 flex-col items-center justify-center px-16 text-center transition-transform duration-700 ease-in-out ${
                                isRegister ? 'translate-x-[20%]' : 'translate-x-0'
                            }`}>
                                <h2 className="mb-6 text-4xl font-extrabold text-white drop-shadow-md tracking-tight">Người mới à?</h2>
                                <p className="mb-10 text-lg text-white/90 leading-relaxed font-light">
                                    Hãy gia nhập cộng đồng của chúng tôi và khám phá những điều tuyệt vời nhất!
                                </p>
                                <button onClick={toggleMode} className="rounded-full border-2 border-white/80 bg-white/10 backdrop-blur-sm px-12 py-3.5 font-bold uppercase tracking-wider text-white transition hover:bg-white hover:text-[#71bc59] shadow-xl">
                                    Tạo tài khoản
                                </button>
                            </div>
                        </div>
